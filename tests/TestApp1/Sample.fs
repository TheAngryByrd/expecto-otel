module Tests

open Expecto
open OpenTelemetry
open OpenTelemetry.Resources
open OpenTelemetry.Trace
open OpenTelemetry.Logs
open OpenTelemetry.Metrics
open System.Diagnostics
open FsOpenTelemetry
open Microsoft.Extensions.Logging
open System.Diagnostics.Metrics

let serviceName = "TestApp1"

let actSource = new ActivitySource(serviceName, "1.0.0")

let meter = new Meter("System.Diagnostics")

let resourceBuilder =
    ResourceBuilder
        .CreateDefault()
        .AddService(serviceName = serviceName)

let loggerFactory = LoggerFactory.Create(fun builder ->
    builder.AddOpenTelemetry(fun logging ->
        logging
            .AddOtlpExporter()
            .SetResourceBuilder(resourceBuilder)
        |> ignore
    )
    |> ignore
)



let meterProvider =
    Sdk
        .CreateMeterProviderBuilder()
        .AddMeter()
        .AddRuntimeInstrumentation()
        .SetResourceBuilder(
            resourceBuilder
        )
        .AddOtlpExporter()
        .Build()

let traceProvider =
        Sdk
            .CreateTracerProviderBuilder()
            .AddSource(serviceName, "Library1")
            .SetResourceBuilder(
                resourceBuilder
            )
            .AddOtlpExporter()
            .Build()

module TracedTests =
    let internal testCase1 name (activityName : string) f =
        testCase name <| fun _ ->
            use activity = actSource.StartActivity(activityName)
            try
                f ()
            with exn ->
                activity.SetStatus(Status.Error)
                activity.RecordException(exn)
                reraise()
    let testCase name f =
        testCase1 name name f

open TracedTests


[<Tests>]
let tests =

  testList "samples" [
    testCase "universe exists (╭ರᴥ•́)" <| fun _ ->
      let logger = loggerFactory.CreateLogger("Tests")
      logger.LogWarning("Universe exists?!?! {hello}", "World" )
      let subject = true
      Expect.isTrue subject "I compute, therefore I am."

    testCase "Universe explodes" <| fun _ ->
      let logger = loggerFactory.CreateLogger("Tests")
      logger.LogWarning("Universe explodes?!?!")
      let subject = true
      Expect.isFalse subject "I compute, therefore I am."

    testCase "Say Hello" <| fun _ ->
      let logger = loggerFactory.CreateLogger("Tests")
      logger.LogWarning("Say Hello?!?!")
      let subject = Library2.Say.hello "World"
      Expect.equal "???" subject "I compute, therefore I am."

    // testCase "when true is not (should fail)" <| fun _ ->
    //   let subject = false
    //   Expect.isTrue subject "I should fail because the subject is false"

    // testCase "I'm skipped (should skip)" <| fun _ ->
    //   Tests.skiptest "Yup, waiting for a sunny day..."

    // testCase "I'm always fail (should fail)" <| fun _ ->
    //   Tests.failtest "This was expected..."

    // testCase "contains things" <| fun _ ->
    //   Expect.containsAll [| 2; 3; 4 |] [| 2; 4 |]
    //                      "This is the case; {2,3,4} contains {2,4}"

    // testCase "contains things (should fail)" <| fun _ ->
    //   Expect.containsAll [| 2; 3; 4 |] [| 2; 4; 1 |]
    //                      "Expecting we have one (1) in there"

    // testCase "Sometimes I want to ༼ノಠل͟ಠ༽ノ ︵ ┻━┻" <| fun _ ->
    //   Expect.equal "abcdëf" "abcdef" "These should equal"

    // test "I am (should fail)" {
    //   "╰〳 ಠ 益 ಠೃ 〵╯" |> Expect.equal true false
    // }
  ]
